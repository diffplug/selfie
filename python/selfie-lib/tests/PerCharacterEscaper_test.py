...
class TestPerCharacterEscaper:
    def test_performance_optimization_self(self):
        ...
        assert escaper.escape(abc) is abc, "Escape should return the original object when no change is made"
        assert escaper.unescape(abc) is abc, "Unescape should return the original object when no change is made"

        # Use '==' for checking value equality.
        assert escaper.escape("1") == "`1", "Escaping '1' should prepend the escape character"
        assert escaper.escape("`") == "``", "Escaping the escape character should duplicate it"
        assert escaper.escape("abc123`def") == "abc`1`2`3``def", "Escaping 'abc123`def' did not produce the expected result"

        assert escaper.unescape("`1") == "1", "Unescaping '`1' should produce '1'"
        assert escaper.unescape("``") == "`", "Unescaping '``' should produce '`'"
        assert escaper.unescape("abc`1`2`3``def") == "abc123`def", "Unescaping 'abc`1`2`3``def' did not produce the expected result"
    ...
    def test_performance_optimization_specific(self):
        ...
        assert escaper.escape("abc123`def") == "abc`b`c`d`adef", "Escaping 'abc123`def' did not produce the expected result"
    ...
    def test_roundtrip(self):
        ...
        def roundtrip(str):
            assert escaper.unescape(escaper.escape(str)) == str, f"Roundtrip of '{str}' did not return the original string"
