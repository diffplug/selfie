class PerCharacterEscaper:
    ...
    @staticmethod
    def _first_offset_needing_escape(input_string, escaped_code_points, escape_code_point=None):
    ...
    def escape(self, input_string):
        no_escapes = self._first_offset_needing_escape(input_string, self.escaped_code_points)
        if no_escapes == -1:
            return input_string
        else:
            result = []
    ...
    def unescape(self, input_string):
        if input_string.endswith(chr(self.escape_code_point)) and not input_string.endswith(chr(self.escape_code_point)*2):
            raise ValueError("Escape character '{}' can't be the last character in a string.".format(chr(self.escape_code_point)))
        
        no_escapes = self._first_offset_needing_escape(input_string, [self.escape_code_point], self.escape_code_point)
        if no_escapes == -1:
            return input_string
        else:
            result = []
