...
A [Lens](https://kdoc.selfie.dev/selfie-lib/com.diffplug.selfie/-lens/) is just a function that transforms one `Snapshot` into another `Snapshot`, transforming / creating / removing values along the way. For example, we might want to pretty-print the HTML in our snapshots.
...
public static Selfie.DiskSelfie expectSelfie(Response response) {
  return Selfie.expectSelfie(response, RESPONSE.withLens(PRETTY_PRINT));
}
public static Selfie.DiskSelfie expectSelfie(Email email) {
  return Selfie.expectSelfie(response, EMAIL.withLens(PRETTY_PRINT));
}
```
...
We can easily mutate an specific facet, such as to pretty-print HTML in the subject...
...
## Mixing disk and inline literals

Snapshot testing has been underused for three reasons:

- controlling read vs write is cumbersome (fixed by [control comments](https://selfie.dev/jvm/get-started#quickstart))
- stale snapshots pile up (fixed by [garbage collection](https://github.com/diffplug/selfie/blob/main/selfie-runner-junit5/src/main/kotlin/com/diffplug/selfie/junit5/SelfieGC.kt))
- a great test should tell a story, and disk snapshots can't do that

Inline snapshots are a partial fix for storytelling within a test, but the harnessing required can be verbose. This is where we combline all of selfie's features:

- exhaustively detailed on disk
- succinct storytelling inline
- eliminate boilerplate and harnessing thanks to `Camera` and `CompoundLens`
...
to persuade you that [assertions are just temporarily embarrassed snapshots](https://thecontextwindow.ai/p/temporarily-embarrassed-snapshots).
