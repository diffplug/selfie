...
You must choose only one, you'll get an error if both are on the classpath. You'll need to add the `SelfieExtension` to your classpath like so:
...
For a JUnit test, `selfie-runner-junit5` uses a `ThreadLocal` to pass information about the currently running test. If you want to take a snapshot on a different thread, you can save that information and use it on any thread using `Selfie.later()`.
...
If for some reason you want to take a snapshot outside of the coroutine, that is possible.
...
This issue of using `Selfie` or `SelfieSuspend` only matters for disk snapshots. So there is a chance for confusion where you might be using inline snapshots (`.toBe`) without any problem. Then you switch to disk snapshots (`.toMatchDisk`) and get an error telling you to switch from `Selfie` to `SelfieSuspend`. The explanation is why.
