...
**TODO: Toy project for Python not [yet](https://github.com/diffplug/selfie/issues/322) implemented, PRs Welcomed.**

We'll be using the [`example-pytest-selfie`](https://github.com/diffplug/selfie/issues/322) project from the selfie GitHub repo. You can clone the code and follow along, but there's no need to. If you did clone the project, you could run **Not Implemented** and you'd have a webapp running at `localhost:8080`.
...
**TODO: Use request instead?, toy project not implemented [yet](https://github.com/diffplug/selfie/issues/322).**
...
public void homepage() {
  expectSelfie(requests.get("/").text).toBe("""
<html><body>
\u0020 <h1>Please login</h1>
...
**TODO: `Camera` and `expectSelfie(T, Camera&lt;T&gt;)` not yet implemented, [PRs welcomed](https://github.com/diffplug/selfie/issues/302)**

The first thing to notice is that we'll be doing a lot of `requests.get().body().text`. It would be nice if we could just do `expectSelfie(get("/"))`, but we'll have to write our own `expectSelfie(requests.models.Response)` method. Selfie gives us [`expectSelfie(T, Camera&lt;T&gt;)`](https://github.com/diffplug/selfie/issues/302) and [`Camera`](https://github.com/diffplug/selfie/issues/302) to do exactly that.
...
**TODO: Toy project not [yet](https://github.com/diffplug/selfie/issues/322) implemented**
